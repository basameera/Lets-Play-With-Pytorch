...
class NeuralNet(nn.Module):
            ...
            if show_progress:
                if batch_idx % int(len(training_loader)*0.05) == 0:
                    print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f}'.format(
                        epoch, batch_idx *
                        len(data), len(training_loader.dataset),
    ...
    def validation_step(self, validation_loader, show_progress=False):
        ...
        with torch.no_grad():
            ...
            # Adding loss to history
            self.valid_loss_hist.append(
                self.valid_loss / len(validation_loader))
...
def cmdArgs():
    parser = argparse.ArgumentParser(
        description='PyTorch NN Template\n- by Bassandaruwan')
...
def main():
    ...
    # print args
    prettyPrint(args.__dict__, 'Args')

    # print settings
    prettyPrint(settings, 'settings')

    # Instantiate mode
    model = NeuralNet(use_cuda=use_cuda)

    # Train model
    model.fit(train_loader, valid_loader, epochs=args.epochs, save_best=args.save_best, show_progress=True, save_plot=False)
